...
export class Darkmoon {
  constructor() {
    ...
    this.craftMediumComplete = document.querySelector(".craft-medium-complete");
    this.craftMediumEven = document.querySelector(".craft-medium-even");
    this.craftMediumOverflow = document.querySelector(".craft-medium-overflow");
    ...
    this.listOrderAscending = document.getElementById("list-order-asecnding");
    this.listOrderDescending = document.getElementById("list-order-desecnding");
  ...
  init() {
    ...
    const minusBtns = [...document.querySelectorAll(".minus-btn")].reverse();
    ...
    this.initNumbers = [...document.querySelectorAll(".init-number")].reverse();
    ...
    this.craftMediumBtn.onclick = () => {
      ...
      if (!Calculator.isValidNumber(medium) || !Calculator.isValidNumber(low)) {
        alert("Invalid input!");
        return;
      }
  ...
  displayMaterial() {
    ...
    for (let i = 0; i < 3; i++) {
      this.materialNumberInputs[i].value = numbers[i] === 0 ? "" : numbers[i];
  ...
  addEnemy(enemy) {
    ...
    let selectLevelHTML = `
            ...
            >`;
    for (let level = this.dm.enemyLevelRanges.length - 1; level >= 0; level--) {
  ...
  displayStatistics() {
    ...
    calculator.calculateRun(this.sucroseBonus.checked, this.doriBonus.checked);
